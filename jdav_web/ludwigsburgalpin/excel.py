"""Excel export for the Ludwigsburg-Alpin programme.

Was ``TerminAdmin.make_overview``, the admin action that wrote the selected
``Termin`` rows into a workbook. It lives here now that the admin is gone, so
the API owns the generator directly and the column layout has a single home.
"""
...
def generate_termin_overview(queryset):
    """Write ``queryset`` into the programme workbook and serve it."""
    ensure_media_dir()
