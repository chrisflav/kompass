...
from members.serializers import FreizeitSerializer
...
class Statement(CommonModel):
    ...
    auto_bill_config = models.ForeignKey(
        "AutomaticBillConfiguration", null=True, on_delete=models.SET_NULL
    )
    ...
    def transaction_issues(self):
        ...
        # automatically generated paiments
        generated_bills = self.auto_bill_config.generate_bills(self)
        for bill in generated_bills:
            needed_paiments.append((bill.paid_by, bill.amount))
...
class StatementSerializer(serializers.ModelSerializer):
    """Serializer for the `Statement` model."""

    excursion = FreizeitSerializer()
    night_cost = serializers.FloatField()
...
class BillGenerator(models.Model):
    """A declarative rule to generate bills for a statement."""
    ...
    rule = models.TextField(
        verbose_name=_("Rule"),
        blank=True,
        help_text=_("Rule generating expenses from the input parameters of this statement."),
    )
    ...
    def generate_bills(self, statement):
        """Generate a list of bills based on the generator rule for the given statement."""
        try:
            serialized_statement = StatementSerializer(statement)
            logger.debug(f"Evaluating {self.rule}.")
            expr = Jsonata(self.rule)
            # Expect: List of `Bill`s
            bills = []
            results = expr.evaluate(serialized_statement.data)
            for result in results:
                bill = GeneratedBillSerializer(
                    data=dict(result, statement=statement.pk, rule=self.pk)
                )
                if bill.is_valid():
                    bills += [GeneratedBill(**bill.validated_data)]
                else:
                    logger.warning(f"Parsed bill is invalid: {bill.errors}")
            return bills
        except Exception as e:
            logger.warning(f"Failed to evaluate JSONata rule: {e}")
            return []
    ...
    class Meta:
        verbose_name = _("Bill generation rule")
        verbose_name_plural = _("Bill generation rules")


class GeneratedBill(Bill):
    generated_at = models.DateTimeField(default=timezone.now, verbose_name=_("Generated at"))
    rule = models.ForeignKey(
        BillGenerator,
        verbose_name=_("Generated by rule"),
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
...
class AutomaticBillConfiguration(models.Model):
    name = models.CharField(verbose_name=_("Name"), max_length=100)
    description = models.TextField(verbose_name=_("Description"))
    generators = models.ManyToManyField(BillGenerator, verbose_name=_("Bill generators"))

    def generate_bills(self, statement, groupby_generator=False):
        bills = []
        for generator in self.generators.all():
            if groupby_generator:
                generated_bills = generator.generate_bills(statement)
                bills += [
                    {
                        "generator": generator,
                        "bills": generated_bills,
                        "total": sum([b.amount for b in generated_bills]),
                    }
                ]
            else:
                bills += generator.generate_bills(statement)
        return bills
...
class GeneratedBillSerializer(serializers.ModelSerializer):
    """Serializer for the `GeneratedBill` model."""
