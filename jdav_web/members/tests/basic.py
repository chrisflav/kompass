...
class PermissionFixtureTestCase(TestCase):
    """Users, groups and per-member ACLs shared by the permission tests.

    Was ``AdminTestCase``, which also built a ``ModelAdmin`` instance; only the
    fixture survived the admin's removal.
    """
...
class PermissionTestCase(PermissionFixtureTestCase):
    def setUp(self):
        super().setUp()
