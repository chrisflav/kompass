...
def generate_group_overview(all_groups, limit_to_public=True):
    ...
    for group in all_groups:
        ...
        yl_count = len(
            [member for member in group.member_set.all() if member in group.leiters.all()]
        )
...
def generate_ljp_vbk(excursion):
    ...
    if not hasattr(excursion, "ljpproposal"):
        raise ValueError("Excursion has no LJP proposal.")
    ...
    sheet["C19"] = (
        LJP_GOALS[excursion.ljpproposal.goal] if excursion.ljpproposal.goal in LJP_GOALS else ""
    )
    ...
    sheet["N19"] = (
        f"{NOT_BW_REASONS[excursion.ljpproposal.not_bw_reason]}"
        if excursion.ljpproposal.not_bw_reason is not None
        else ""
    )
    ...
    name = normalize_filename(
        f"{excursion.code}_{title}_LJP_V-BK_3.{excursion.ljpproposal.category}", date=excursion.date
    )
    filename = name + ".xlsx"
