...
class CommonAdminMixinTestCase(TestCase):
    ...
    def test_formfield_for_dbfield_with_formfield_overrides(self):
        ...
        result = admin_instance.formfield_for_dbfield(
            db_field, request, help_text="Test help text"
        )
    ...
    @override_settings(
        CUSTOM_MODEL_FIELDS={
            "contrib_dummymodel": {"fields": ["field1", "field3", "field5"]}
        }
    )
    ...
    def test_included_and_excluded_fields(self):
        ...
        excluded = self.admin.get_exclude(self.request)
        #  custom fields should take precedence over exclude
    ...
    @override_settings(
        CUSTOM_MODEL_FIELDS={
            "contrib_dummymodel": {"fields": ["field5", "field3", "field1"]}
        }
    )
    ...
    def test_nonexistent_fields(self):
        """Test behavior with nonexistent fields in settings. They should be included
        so that the admin shows an error about missing fields."""
        with override_settings(
            CUSTOM_MODEL_FIELDS={
                "contrib_dummymodel": {"fields": ["field1", "nonexistent_field"]}
            }
        ):
            fields = self.admin.get_fields(self.request)
            ...
            self.assertEqual(set(exclude), set())

    def test_nonexistent_exclude(self):
        """Test behavior with nonexistent fields in settings. They should be kept so that
        the admin shows an error about missing fields."""
        with override_settings(
            CUSTOM_MODEL_FIELDS={
                "contrib_dummymodel": {"exclude": ["field1", "nonexistent", "field2"]}
            }
        ):
            fields = self.admin.get_fields(self.request)
    ...
    def test_empty_settings(self):
        """Test behavior with empty settings, there should be no effect"""
        """Test behavior with empty settings, there should be no effect"""
        fields = self.admin.get_fields(self.request)
        self.assertEqual(fields, ["field1", "field2", "field3", "field4", "field5"])
    ...
    @override_settings(CUSTOM_MODEL_FIELDS={"contrib_dummymodel": {"fields": []}})
    ...
    @override_settings(
        CUSTOM_MODEL_FIELDS={"contrib_dummymodel": {"exclude": ["field5"]}}
    )
    ...
    @override_settings(
        CUSTOM_MODEL_FIELDS={
            "contrib_dummymodel": {"exclude": ["field3", "field4", "field5"]}
        }
    )
    ...
    def test_default_with_exclude(self):
        """Test that admin exclude is respected by default"""
    ...
    def test_override_exclude_with_exclude(self):
        ...
        exclude = admin_instance.get_exclude(self.request)
        self.assertEqual(set(exclude), {"field5", "field2", "field4"})
